...
def update_pc_select_group(active_program, channel=0):
    """Режим select (радиогруппа) только для PC-кнопок.
    Зажигает только одну кнопку, соответствующую program.
    Все остальные кнопки с mode="select" гасятся.
    """
    try:
...
def _process_midi_msg(msg, source="USB"):
    """Process a received MIDI message Ч update LED/button state."""
    if not msg:
        return
